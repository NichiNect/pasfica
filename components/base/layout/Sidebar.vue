...
<template>
  <div class="container mx-auto grid min-h-screen grid-cols-7 lg:grid-cols-9 gap-8">
    <div :class="isClose ? 'absolute scale-x-0 -translate-x-full py-4' : 'lg:col-span-2 scale-100 py-4'">
      <div class="flex flex-col h-full bg-white rounded-xl overflow-hidden shadow">
        <div class="m-4 lg:px-6 py-4 flex justify-center lg:justify-start gap-8 bg-primary rounded-xl">
          ...
          <h1 class="text-xl font-bold text-white block lg:hidden">MP</h1>
        </div>

        <div class="max-h-[calc(100vh-130px)] p-2 mb-4 overflow-y-auto scroll_control">
          <div v-for="(menuGroup, menuGroupKey) in props.menu">
            <div>
              <div class="px-2 lg:px-4 pt-4 pb-2 flex justify-between cursor-pointer" @click="(e) => {
              ...
              <div :class="[!menuGroup.collapse || showMenu.includes(menuGroupKey) ? '' : 'hidden']">
                <div v-for="(menuItem, menuItemKey) in menuGroup.submenu">
                  <a :class="[
                    'flex items-center duration-150 justify-between gap-4 py-4 my-2 cursor-pointer', 
                      ...
                      <div v-for="(subMenuItem, subMenuItemKey) in menuItem.submenu">
                        <NuxtLink :to="`${basePath}${subMenuItem.path}`" :key="subMenuItemKey">
                          ...
                          >
                            ...
                            <h6 class="font-semibold hidden lg:block">
                              {{ subMenuItem.label }}
                            </h6>
                          </div>
                        </NuxtLink>
