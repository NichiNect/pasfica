...
<template>
  <div class="grid min-h-screen grid-cols-6 lg:grid-cols-12 gap-8 mx-4">
    <div :class="isClose ? 'absolute scale-x-0 -translate-x-full py-4' : 'absolute z-10 top-24 scale-100 col-span-8 py-4 lg:relative lg:top-0 lg:col-span-2'">
      <div class="flex flex-col h-full bg-primary rounded-xl overflow-hidden shadow-2xl lg:shadow">
        ...
        <div class="bg-white rounded-xl py-0.5 mx-4"></div>
          ...
          <div v-for="(menuGroup, menuGroupKey) in props.menu">
            <div>
              <div class="px-2 lg:px-4 pt-4 pb-2 flex justify-between cursor-default" @click="(e) => {
                ...
                <h6 class="text-sm lg:text-lg font-medium text-white dark:text-white hidden lg:block">{{ menuGroup.head }}</h6>
                <div class='w-full h-1 bg-gray-300 rounded-full block lg:hidden'></div>
                <div v-if="menuGroup.collapse">
                  <FontAwesomeIcon :icon="faChevronDown" :class="['text-lg text-white dark:text-white', showMenu[0] && showMenu.includes(menuGroupKey) ? 'rotate-180' : '']" />
                </div>
              </div>
                ...
                <div v-for="(menuItem, menuItemKey) in menuGroup.submenu">
                  <a :class="[
                    ...
                    menuItem?.submenu?.filter((val) => val.path == router.currentRoute.value.path)[0] || (router.currentRoute.value.path == `${basePath}${menuItem.path}`)
                      ? 'text-white bg-secondary mx-2 lg:mx-4 lg:px-4 rounded-xl'
                      : 'lg:px-8 text-white hover:text-secondary'
                    ]"
    ...
    <div :class="[isClose ? 'col-span-12' : 'col-span-6 lg:col-span-10', 'py-4 max-h-screen overflow-auto scroll_control']">
      <slot />
    </div>
  </div>
</template>
